...
def verifica_diferenca_ctg_validacao_tipo (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
...
def verifica_diferenca_qtd_validacao_tipo (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	""" Verifica se há diferença no contagem total de fichas """
	return df_extraido['Validação'].count() - df_tratado['validacao_nome'].count()

def verifica_diferenca_qtd_validacoes_total (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
