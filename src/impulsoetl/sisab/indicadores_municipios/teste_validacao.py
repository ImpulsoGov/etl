...
def verifica_diferenca_mun_betim (df:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	...
	return df.query("ibge == '310670'")["numerador"].astype(int).sum() - df_tratado.query("municipio_id_sus == '310670'")["numerador"].sum()
...
def verifica_diferenca_qtd_numerador (df:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	""" verifica_diferenca_qtd_numerador: Verifica se há diferença no somatório de numerador """
	return df['numerador'].astype(int).sum() - df_tratado['numerador'].sum()

def verifica_diferenca_qtd_denominador_informado (df:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	""" verifica_diferenca_qtd_denominador_informado: Verifica se há diferença no somatório de denominador informado """
	return df['denominador_informado'].astype(int).sum() - df_tratado['denominador_informado'].sum()

def verifica_diferenca_qtd_denominador_estimado (df:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	""" verifica_diferenca_qtd_denominador_estimado: Verifica se há diferença no somatório de denominador estimado """
	return df['denominador_estimado'].astype(int).sum() - df_tratado['denominador_estimado'].sum()

def verifica_diferenca_qtd_nota (df:pd.DataFrame,df_tratado:pd.DataFrame) -> int:
	""" verifica_diferenca_qtd_nota: Verifica se há diferença no somatório de nota"""
	return df['nota'].astype(int).sum() - df_tratado['nota_porcentagem'].sum()

def verifica_validade_nota (df_tratado:pd.DataFrame):
    """ verifica_validade_nota: Verifica se os valores de nota está entre 0 e 100"""
...
def verifica_validade_numerador(df_tratado:pd.DataFrame) -> int:
	""" verifica_validade_numerador: Verifica se os valores de numerador é menor que denominador estimado"""
	for index, row in df_tratado.iterrows():
            return row["denominador_estimado"] - row["numerador"] 

def teste_validacao(df:pd.DataFrame,df_tratado:pd.DataFrame,indicador:str):
