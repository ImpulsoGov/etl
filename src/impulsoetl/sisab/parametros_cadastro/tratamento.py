...
def tratamento_dados(
    ...
    if nivel_agregacao == "estabelecimentos_equipes":
        tabela_consolidada[
            ["municipio_id_sus", "cnes_id", "cnes_nome", "ine_id", "parametro"]
        ] = dados_sisab_cadastros.loc[
            :, ["IBGE", "CNES", "Nome UBS", "INE", "parametro"]
        ]
        tabela_consolidada["cnes_id"] = tabela_consolidada["cnes_id"].astype(
            "string"
        )
        tabela_consolidada["cnes_nome"] = tabela_consolidada[
            "cnes_nome"
        ].astype("string")
        ...
        tabela_consolidada[
            ["municipio_id_sus", "parametro"]
        ] = dados_sisab_cadastros.loc[:, ["IBGE", "parametro"]]
    ...
    tabela_consolidada["parametro"] = tabela_consolidada["parametro"].astype(
        int
    )
