...
def verifica_diferenca_ctg_uf (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> bool:
	""" Verifica se há diferença na contagem de estados """
	return (
		df_extraido['uf_sigla'].nunique() - df_tratado['uf_sigla'].nunique() == 0
	)

def verifica_diferenca_ctg_municpios (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> bool:
...
def verifica_diferenca_qtd_registros (df_extraido:pd.DataFrame,df_tratado:pd.DataFrame) -> bool:
